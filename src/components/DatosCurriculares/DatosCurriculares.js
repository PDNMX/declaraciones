        ...
        <Grid container spacing={24}>
            ...
            <Grid item xs={2}>
              <FormControl className={classes.select}>
                ...
                <Select
                  value={
                    data.datos_curriculares_grados_academicos
                      .lugar_institucion_educativa.entidad_federativa.cve_agee
                  }
                  onChange={handleChange("lugar_institucion_educativa.entidad_federativa")}
                >
                  {data.catEntidadesFederativas.map(dato => (
                    <MenuItem key={dato.cve_agee} value={dato.cve_agee}>
                      {dato.nom_agee}
                    </MenuItem>
                  ))}
                </Select>
          ...
          <Grid item xs={2}>
            <FormControl className={classes.select}>
              ...
              >
                {data.catEstatusEstudio.map(dato => (
                  <MenuItem key={dato.codigo} value={dato.codigo}>
                    {dato.valor}
                  </MenuItem>
                ))}
          ...
          <Grid item xs={2}>
            <FormControl className={classes.select}>
              ...
              >
                {data.catDocumentoObtenido.map(dato => (
                  <MenuItem key={dato.codigo} value={dato.codigo}>
                    {dato.valor}
                  </MenuItem>
                ))}
