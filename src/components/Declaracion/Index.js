...
class Index extends Component {
  ...
  getNomEntidad = cve_agee => {
    let entidad = this.state.catEntidadesFederativas.filter(x => x.cve_agee === cve_agee);
    return entidad[0].nom_agee;
  };

  getEntidadFederativa = cve_agee => {
    let info = this.state.catEntidadesFederativas.filter(
      x => x.cve_agee === cve_agee
    );
    this.updateMunicipios(cve_agee);
  ...
  //   const valor = this.state.catEntidadesFederativas.filter(
  //     x => x.cve_agee === event.target.value
  //   );
  //
  //   this.setState({
  //     [name]: {
  //       nom_agee: valor[0].nom_agee,
  ...
  // var catPaises = event.target.value;
  // var nacionalidad = [];
  ...
  setDataDeudas = field => event => {
    ...
    switch (field) {
      ...
      case "municipio":
        ...
        this.updateLocalidades(
          this.state.datos_deudas.domicilio_acreedor.entidad_federativa.cve_agee,
          valor
        );
        break;
      case "cp":
  ...
  componentDidMount() {
    ...
    fetch(config.apiHost + "catEntidadesFederativas")
      .then(res => res.json())
      .then(catEntidadesFederativas => this.setState({ catEntidadesFederativas: catEntidadesFederativas }));
    ...
    fetch(config.apiHost + "catTipoVialidad")
      .then(res => res.json())
      .then(catTipoVialidad => this.setState({ catTipoVialidad: catTipoVialidad }));
